...
description: >
  Companion skill for iOS, Android, and Unity k-ID integrations — collects platform-supplied age signals (iOS Declared Age Range, Google Play Families, Unity user data) before the age gate, passes them on `/age-gate/check` with correct trust levels, and renders every k-ID widget URL (age gate, end-to-end, consent, age verification, manage-permissions, data notices) in a system-browser context — Android **Custom Tabs** or iOS **ASWebAuthenticationSession** (`SFSafariViewController` is also acceptable on iOS). Strongly recommends against Android `WebView` and iOS `WKWebView`: neither supports WebAuthn, so neither can create or use k-ID **AgeKeys** (FIDO-based reusable age-proof credentials), which is both a material UX downgrade and a compliance limitation in jurisdictions where AgeKeys are relied on for "highly effective age assurance". Uses the universal `redirectUrl` deep-link callback pattern for result delivery (works across all browser contexts, reliable even when the app backgrounds), and reserves DOM `postMessage` events for the WebView/WKWebView fallback only. Also covers deep-link returns from Family Connect / asktoplay.com and third-party verification apps (for example ConnectID opening the user's banking app). Use when the app is native (not just responsive web), when the user asks about App Store Accountability Acts, Google Play Families, Unity, or AgeKeys, or when verification or consent doesn't work on iOS Safari popups. Not for browser-only web integrations — their platform signals come from headers, not native APIs.
