...
description: >
  Implements the parental-consent flow for k-ID minors — the GUARDIAN-managed path where a trusted adult grants permissions on behalf of a child (COPPA parental consent, EU GDPR-Kids verifiable parental consent, UK AADC equivalent). Supports both integration approaches: Pattern A (the default) builds a fully custom consent screen (QR + OTP + email + direct link) and calls /challenge/send-email and /challenge/generate-otp directly with top-level polling, producing a brand-fit, inline consent experience, and Pattern B is a fast-path fallback that uses the k-ID end-to-end widget (/widget/generate-e2e-url — one iframe handles the age gate, parental consent, data notices, permissions and preferences together) or the manage-permissions widget (/widget/generate-manage-session-permissions-url — post-gate consent only) when the integration must be small, simple, and fast to ship. Use when building the consent screen, wiring challenge send or status endpoints, handling `challengeId` without a session, or diagnosing why consent approvals do not propagate to the app. Not for age assurance or threshold verification where a user must prove their own age (see k-id-age-verification) and not for the age gate itself (see k-id-age-gate).
