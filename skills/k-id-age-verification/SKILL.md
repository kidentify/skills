...
description: >
  Implements k-ID age verification and age assurance for all three patterns the k-ID CDK supports — (1) standalone AgeKit+ age assurance with no k-ID session (single age-proof decision for 18+ sites under UK Online Safety Act, age-restricted downloads, Australia social media minimum-age checks), (2) unverified-adult verification in a sessioned integration (prove you're an adult after the age gate), and (3) per-permission threshold verification for high-risk features (Brazil ECA Digital loot-boxes / targeted ads / direct marketing, UK OSA 18+ permissions, Australia social media, and any other verifiedAgeThreshold regime). Covers /age-verification/perform-access-age-verification (with or without sessionId), /session/upgrade returning CHALLENGE_SESSION_UPGRADE_BY_AGE_ASSURANCE, iframe modal (never window.open), an in-app verification prompt, new-tab return-path detection (server poll + BroadcastChannel + visibility wake in parallel), result handling via webhook or polling, and race-free session update. References AgeKit+ waterfall and ConnectID (Australia) for regional methods. Use when adding AgeKit+ standalone verification, when a session is unverified, when a permission has verifiedAgeThreshold, when meeting "highly effective age assurance" requirements, when building the verification UI, or when diagnosing "user verified but app never unlocked". Not for the initial claimed-age check in a sessioned integration (see k-id-age-gate) and not for parental consent for minors (see k-id-consent-and-challenges).
...
- [`verification.revoke` webhook](https://docs.k-id.com/events/webhooks/event-types/verification-revoke)
